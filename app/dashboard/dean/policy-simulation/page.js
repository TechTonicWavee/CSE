...
const POLICY_RULES = {
  "Add Course": (config) => {
    ...
    const isCommunication =
      course.includes("commun") ||
      course.includes("soft") ||
      course.includes("verbal");
    const isTechnical =
      course.includes("tech") ||
      course.includes("data") ||
      course.includes("algo");
    const isAnalytical =
      course.includes("analy") ||
      course.includes("math") ||
      course.includes("stat");

    return {
      placementDelta: +(
        (isCommunication ? 8 : isTechnical ? 6 : 5) *
        m *
        bs
      ).toFixed(1),
...
function runSimulation(config) {
  ...
  const after = {
    placementReadiness: Math.min(
      99,
      BASE.placementReadiness + delta.placementDelta,
    ),
    ...
    technicalSkills: Math.min(
      99,
      BASE.technicalSkills + delta.technicalSkillsDelta,
    ),
    analyticalSkills: Math.min(
      99,
      BASE.analyticalSkills + delta.analyticalDelta,
    ),
    ...
    industryReadiness: Math.min(
      99,
      BASE.industryReadiness + delta.industryDelta,
    ),
  };
  ...
  const trend = [0, 1, 2, 3, 4, 5].map((i) => ({
    ...
    after: +(
      BASE.placementReadiness +
      delta.placementDelta * (i === 0 ? 0 : i / 3) +
      i * 1.1
    ).toFixed(1),
  ...
  if (delta.placementDelta >= 10)
    insights.push({
      type: "positive",
      text: `Placement readiness improves by ${delta.placementDelta}% — a significant uplift for the current batch.`,
    });
  else if (delta.placementDelta > 0)
    insights.push({
      type: "positive",
      text: `Placement readiness improves by ${delta.placementDelta}%, benefiting ~${Math.round((1240 * delta.placementDelta) / 100)} students directly.`,
    });

  if (Math.abs(delta.atRiskDelta) >= 1.5)
    insights.push({
      type: "positive",
      text: `At-risk student count reduces by ~${Math.round((1240 * Math.abs(delta.atRiskDelta)) / 100)} students (${Math.abs(delta.atRiskDelta).toFixed(1)}% reduction).`,
    });

  const bestBranch = branchImpact.sort(
    (a, b) => b.after - b.before - (a.after - a.before),
  )[0];
  const worstBranch = [...branchImpact].sort(
    (a, b) => a.after - a.before - (b.after - b.before),
  )[0];
  insights.push({
    type: "info",
    text: `${bestBranch.branch} shows highest impact (+${(bestBranch.after - bestBranch.before).toFixed(1)}%) due to stronger baseline infrastructure.`,
  });
  if (worstBranch.branch !== bestBranch.branch)
    insights.push({
      type: "warning",
      text: `${worstBranch.branch} shows lowest response (+${(worstBranch.after - worstBranch.before).toFixed(1)}%) — may require additional targeted interventions.`,
    });

  if (delta.communicationDelta >= 10)
    insights.push({
      type: "positive",
      text: `Communication skills see the sharpest improvement (+${delta.communicationDelta} pts), directly boosting interview performance.`,
    });
  ...
  insights.push({
    type: "info",
    text: semBreak
      ? "Earlier-year intervention maximises compounded impact over remaining semesters."
      : "Final-year intervention produces short-term gains; earlier rollout would increase ROI.",
  });

  const recommendation =
    delta.placementDelta >= 8
...
export default function PolicySimulation() {
  ...
  const handleSave = () => {
    ...
    setSaved((prev) => [
      ...prev.slice(-4),
      { label, impactLevel, targetBranch, result, id: Date.now() },
    ]);
  };
  ...
  const handleCompareLoad = (s) => {
    setCompareB(s.result);
    setCompareMode(true);
  };
  ...
  const radarData = result
    ? [
        {
          skill: "Soft Skills",
          before: result.before.softSkills,
          after: result.after.softSkills,
        },
        {
          skill: "Technical",
          before: result.before.technicalSkills,
          after: result.after.technicalSkills,
        },
        {
          skill: "Analytical",
          before: result.before.analyticalSkills,
          after: result.after.analyticalSkills,
        },
        {
          skill: "Communication",
          before: result.before.communication,
          after: result.after.communication,
        },
        {
          skill: "Projects",
          before: result.before.projectWork,
          after: result.after.projectWork,
        },
        {
          skill: "Industry",
                        ...
                        >
                          ...
                          <div className="flex items-center gap-1.5">
                            <span className="text-xs font-bold text-green-600">
                              +{s.result.delta.placementDelta}%
                            </span>
                            <button
                              onClick={() => handleCompareLoad(s)}
                      ...
                      <div className="bg-indigo-50 border border-indigo-200 rounded-2xl p-4 flex items-center gap-3">
                        ...
                        <div className="flex-1">
                          ...
                          <p className="text-xs text-indigo-600">
                            Current scenario:{" "}
                            <strong>
                              +{result.delta.placementDelta}% placement
                            </strong>{" "}
                            &nbsp;·&nbsp; Saved scenario:{" "}
                            <strong>
                              +{compareB.delta.placementDelta}% placement
                            </strong>
                          </p>
                        </div>
                        <button
                      ...
                      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-5">
                        ...
                        <ResponsiveContainer width="100%" height={180}>
                          <BarChart
                            data={
                              compareMode && compareB
                                ? result.branchImpact.map((b, i) => ({
                                    ...b,
                                    compare: compareB.branchImpact[i]?.after,
                                  }))
                                : result.branchImpact
                            }
                        ...
                        <div className="space-y-2.5">
                          {result.insights.map((ins, i) => {
                            ...
                            return (
                              <div
                                key={i}
                                className={`flex gap-2.5 p-3 rounded-xl border ${styles.bg}`}
                              >
                                <div
                                  className={`w-2 h-2 rounded-full mt-1.5 flex-shrink-0 ${styles.dot}`}
                                />
                                <p
                                  className={`text-xs leading-relaxed ${styles.text}`}
                                >
                                  {ins.text}
                                </p>
                              </div>
