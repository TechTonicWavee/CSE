...
import {
  Home, Grid, Users, TrendingUp, BookOpen, Activity, FileText, Target,
  Brain, Settings, LogOut, Search, Bell, ChevronDown, AlertTriangle,
  Briefcase, ArrowUpRight, ExternalLink, Star, ArrowUp, ArrowDown,
  BarChart2, CheckCircle2, ShieldAlert, Zap,
} from 'lucide-react'

const navLinks = [
  { id: 'dashboard',    label: 'Dashboard',           icon: Home,       path: '/dashboard/dean' },
  { id: 'department',   label: 'Department Overview', icon: Grid,       path: '/dashboard/dean/department' },
  { id: 'faculty',      label: 'Faculty Performance', icon: Users,      path: '/dashboard/dean/faculty-performance' },
  { id: 'forecast',     label: 'Cohort Forecasting',  icon: TrendingUp, path: '/dashboard/dean/forecasting' },
  { id: 'curriculum',   label: 'Curriculum Analysis', icon: BookOpen,   path: '/dashboard/dean/curriculum' },
  { id: 'policy',       label: 'Policy Simulation',   icon: Activity,   path: '/dashboard/dean/policy-simulation' },
  { id: 'reports',      label: 'Reports',             icon: FileText,   path: '/dashboard/dean/reports' },
  { id: 'cross',        label: 'Year-wise Insights',  icon: Target,     path: '/dashboard/dean/cross-branch' },
  { id: 'intelligence', label: 'Student Intelligence',icon: Brain,      path: '/dashboard/dean/student-intelligence' },
]
...
export default function DeanDashboard() {
  ...
  return (
    <div className="flex h-screen bg-[#F3F4F6] overflow-hidden font-sans">
      ...
      {/* SIDEBAR */}
      <aside className={`${sidebarOpen ? 'w-[232px]' : 'w-0 overflow-hidden'} flex-shrink-0 bg-white border-r border-gray-100 flex flex-col transition-all duration-300`}>
        <div className="px-4 py-5 border-b border-gray-100">
          <div className="flex items-center gap-3">
            ...
            <div className="overflow-hidden">
              <p className="font-semibold text-[13px] text-[#0D1B2A] truncate leading-tight">Dr. Vineet Sharma</p>
              <p className="text-[11px] text-gray-400 truncate mt-0.5">Head of Department · CSE</p>
            </div>
          </div>
        </div>

        <nav className="flex-1 px-2 py-3 overflow-y-auto">
          <p className="px-3 pb-2 text-[10px] font-semibold uppercase tracking-widest text-gray-400">Main Menu</p>
          {navLinks.slice(0, 6).map(link => (
            <button key={link.id} onClick={() => navigate(link)}
              className={`nav-link w-full text-left mb-0.5 ${activeNav === link.id ? 'active' : ''}`}>
              <link.icon size={16} strokeWidth={activeNav === link.id ? 2.2 : 1.8} />
              <span className="flex-1 truncate">{link.label}</span>
            </button>
          ))}
          <p className="px-3 pt-3 pb-2 text-[10px] font-semibold uppercase tracking-widest text-gray-400">Intelligence</p>
      ...
      <div className="flex-1 flex flex-col overflow-hidden min-w-0">
        ...
        <header className="bg-white border-b border-gray-100 px-5 h-14 flex items-center gap-4 flex-shrink-0">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <span className="font-bold text-[#0D1B2A] text-[13px] tracking-tight hidden sm:block">Educator Analytics OS</span>
          </div>
          ...
          <div className="flex items-center gap-2 cursor-pointer group px-1.5 py-1 rounded-lg hover:bg-gray-100 transition">
            <div className="w-8 h-8 rounded-full flex items-center justify-center text-white font-bold text-[11px]"
              style={{ background: 'linear-gradient(135deg, #5B21B6 0%, #4C1D95 100%)' }}>VS</div>
            <ChevronDown size={13} className="text-gray-400 group-hover:text-gray-600 transition" />
          </div>
        </header>
        ...
        <main className="flex-1 overflow-y-auto px-6 py-6">
          ...
          <div className="mb-6 animate-fade-in flex items-center justify-between">
            <div>
              <h1 className="text-xl font-bold text-[#0D1B2A] tracking-tight">Good morning, Dr. Vineet Sharma</h1>
              <p className="text-[13px] text-gray-500 mt-0.5">CSE Department overview · April 2026</p>
            </div>
